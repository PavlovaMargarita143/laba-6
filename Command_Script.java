...
public class Command_Script implements  Command, Serializable {
    public File file;
        ...
        while (line != null) {
            ...
            if (w[0].compareTo("RemoveAt") == 0) {
               ...
               while (true){
                ...
                if (w.length < 12) {
                    s.add("Не хватает данных для выполнения команды UpdateId, пожалуйста, исправьте данные файла, строка должна содержать минимум 8 значений после названия команды, последнее число является ID обновляемого элемента");
                    continue;
                }
                boolean result = w[2].matches("\\d+");
                if (result == false) {
